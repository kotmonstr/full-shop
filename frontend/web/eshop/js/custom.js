...
/*
 Удаляет товар в списка сравнения
 */
function RemoveFromCompareList(good_id) {
    var csrf_token = $("meta[name=csrf-token]").attr("content");
    $.ajax({
        url: '/shop/remove-from-comparelist',
        type: 'POST',
        dataType: 'json',
        cache: false,
        asinc: false,
...
function animateFlyToCart(e, good_id) {
    ...
    ImgClone.css("position", "fixed");
    ImgClone.css("z-index", "100");
    ImgClone.css("top", "45%");
    ImgClone.css("left", "45%");
...
var Setup = {
    // смена валюты
    CangeCurrency: function (cur) {
        var csrf_token = $("meta[name=csrf-token]").attr("content");
        $.ajax({
            url: '/shop/change-currency',
            type: 'POST',
            dataType: 'json',
            cache: false,
            asinc: false,
            ...
            success: function (data) {
                $('.currency').html(data.new_currency);
                window.location.reload();
            }
        });
    },
    // смена языка
    CangeLang: function (lang) {
        var csrf_token = $("meta[name=csrf-token]").attr("content");
        $.ajax({
            url: '/shop/change-lang',
            type: 'POST',
            dataType: 'json',
            cache: false,
            asinc: false,
...
function totalRecalculate(){
    ...
    $.ajax({
        ...
        success: function (data) {
            $('.total-table').html(data);
        }
    });
}

// нажатие кнопки создать заявку
function startOrderWindow(){
    $('#cart_items').hide('slow');
    $('.startorder').hide('slow');
    $('#do_action').show('slow');
}
